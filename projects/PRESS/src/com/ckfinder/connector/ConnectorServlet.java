...
public class ConnectorServlet extends HttpServlet {
	...
	private void getResponse(final HttpServletRequest request,
		...
		try {
			...
			if (post
					&& !CommandHandlerEnum.FILEUPLOAD.toString()
										.equals(command.toUpperCase())
					&& !CommandHandlerEnum.QUICKUPLOAD.toString()
										.equals(command.toUpperCase())) {
			...
			if (configuration.getEvents() != null) {
				if (configuration.getEvents().run(EventTypes.BeforeExecuteCommand,
						args, configuration)) {
					CommandHandlerEnum.valueOf(command.toUpperCase()).execute(
	...
	public void init() throws ServletException {
		new ServletContextFactory(getServletContext());
		IConfiguration configuration = null;
		try {
		...
		ErrorUtils.setLangDir(getServletContext());
	}

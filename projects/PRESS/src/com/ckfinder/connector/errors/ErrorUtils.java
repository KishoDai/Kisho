...
public final class ErrorUtils {
	 ...
	 */
	public static void setLangDir(final ServletContext sc) {
		libDir = new File(sc.getRealPath("/WEB-INF/lib/"));
	...
	private List<String> getAllLangCodes() {
		...
		if (libDir.exists() && libDir.isDirectory()) {
			for (String lib : libDir.list()) {
				if (checkConnectorLib(lib)) {
					langs.addAll(readJarFiles(lib));
				}
			}
		}
		return langs;
	...
	/**
	 *
	 * @param lib jar lib name
	 * @return read files from jar that matches lang file pattern.
	 */
	private List<String> readJarFiles(final String lib) {
		List<String> langFiles = new ArrayList<String>();
		try {
			JarFile jarFile = new JarFile(new File(libDir, lib));
	...
	/**
	 * check is lib is ck connector jar.
	 * @param lib lib to check
	 * @return true if jar is CKFinder jar.
	 */
	private boolean checkConnectorLib(final String lib) {
		return Pattern.compile("CKFinder.+\\.jar")
										.matcher(lib).matches();
	}
